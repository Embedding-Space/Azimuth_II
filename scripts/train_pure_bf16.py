#!/usr/bin/env python3
"""
Training worker: pure bf16 initialization.

Initializes embeddings directly in bfloat16 (no float32 stage).
...
def main():
    ...
    print(f"{'='*80}")
    print(f"Pure BF16 Training Worker")
    print(f"{'='*80}")
    ...
    # Pure BF16 initialization (CONTROL)
    print(f"Initializing: pure bf16 (σ={INIT_SIGMA:.2e})")
    with torch.no_grad():
        # Generate directly in bfloat16 (quantization during generation)
    ...
    save_dict = {
        ...
        'init_method': torch.tensor(1, dtype=torch.int32),  # 1=pure_bf16
